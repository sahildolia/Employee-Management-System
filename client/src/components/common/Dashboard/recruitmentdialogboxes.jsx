...
import {
  HandleGetHRRecruitment,
  HandlePostHRRecruitment,
  HandlePatchHRRecruitment,
  HandleDeleteHRRecruitment,
} from "../../../redux/Thunks/HRRecruitmentPageThunk.js";
...
export const CreateRecruitmentDialogBox = ({ children }) => {
  ...
  const CreateRecruitment = async () => {
    const result = await dispatch(
      HandlePostHRRecruitment({ apiroute: "CREATE", data: formdata }),
    );
    if (result.meta.requestStatus === "fulfilled") {
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: result.payload?.message || "Failed to create recruitment.",
      });
    }
  };
          ...
          <div className="form-container grid md:grid-cols-2 min-[250px]:grid-cols-1 gap-4">
                    ...
                    <CommandList>
                      ...
                      <CommandGroup heading="Departments">
                        {HRDepartmentState.data
                          ? HRDepartmentState.data.map((item, index) => (
                              <CommandItem
                                key={index}
                                onSelect={() =>
                                  SelectDepartment(item._id, item.name)
                                }
                              >
                                <span>{item.name}</span>
                              </CommandItem>
            ...
            <div className="form-group flex flex-col gap-3">
              <div className="label-input-field flex flex-col gap-1 h-full">
                ...
                <textarea
                  id="description"
                  className="border-2 border-gray-700 rounded px-2 py-1 h-[200px] resize-none"
                  name="description"
...
export const ViewRecruitmentDialogBox = ({ recruitmentData, children }) => {
  return (
    <Dialog>
        ...
        <div className="recruitment-data-container flex flex-col gap-4">
          ...
          <div className="recruitment-description border-2 border-gray-300 rounded-lg p-4 min-h-[100px]">
            <p className="text-gray-800 whitespace-pre-wrap">
              {recruitmentData.description || "No description provided."}
            </p>
          </div>
          <div className="recruitment-meta text-sm text-gray-500">
            {recruitmentData.createdAt && (
              <p>
                Posted:{" "}
                {new Date(recruitmentData.createdAt).toLocaleDateString()}
              </p>
            )}
          </div>
        </div>
...
export const UpdateRecruitmentDialogBox = ({ recruitmentData, children }) => {
  const dispatch = useDispatch();
  const { toast } = useToast();
  const HRDepartmentState = useSelector(
    (state) => state.HRDepartmentPageReducer,
  ...
  const [selectedDepartment, setSelectedDepartment] = useState(
    recruitmentData.department?.name || null,
  );
  const [formdata, setformdata] = useState({
  ...
  const UpdateRecruitment = async () => {
    const result = await dispatch(
      HandlePatchHRRecruitment({ apiroute: "UPDATE", data: formdata }),
    );
    if (result.meta.requestStatus === "fulfilled") {
      toast({
        title: "Success!",
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: result.payload?.message || "Failed to update recruitment.",
      });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      {/* <DialogTrigger
        className="btn-sm btn-blue-700 text-md border-2 border-blue-800 min-[250px]:px-2 min-[250px]:py-1 sm:px-1 sm:py-0.5 xl:px-2 xl:py-1 rounded-md hover:bg-blue-800 hover:text-white"
        onClick={openDialog}
      >
        Edit
      </DialogTrigger> */}
            <DialogTrigger onClick={openDialog} asChild>{children}</DialogTrigger>
      <DialogContent className="max-w-[95vw] lg:max-w-[50vw] 2xl:max-w-[40vw]">
            ...
            <div className="form-group flex flex-col gap-3">
              <div className="label-input-field flex flex-col gap-1">
                ...
                <input
                  ...
                  className="border-2 border-gray-700 rounded px-2 py-1"
                  name="jobtitle"
                  value={formdata.UpdatedData.jobtitle}
                  onChange={handleformchange}
                />
              </div>
              <div className="label-input-field flex flex-col gap-1">
            ...
            <div className="form-group flex flex-col gap-3">
              <div className="label-input-field flex flex-col gap-1 h-full">
                ...
                <textarea
                  ...
                  className="border-2 border-gray-700 rounded px-2 py-1 h-[200px] resize-none"
                  name="description"
                  value={formdata.UpdatedData.description}
                  onChange={handleformchange}
                ></textarea>
              </div>
            </div>
...
export const DeleteRecruitmentDialogBox = ({ recruitmentID, children }) => {
  const dispatch = useDispatch();
  const { toast } = useToast();

  const DeleteRecruitment = async (RID) => {
    const result = await dispatch(
      HandleDeleteHRRecruitment({ apiroute: `DELETE.${RID}` }),
    );
    if (result.meta.requestStatus === "fulfilled") {
      toast({
        title: "Success!",
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: result.payload?.message || "Failed to delete recruitment.",
      });
    }
  };
          ...
          <div className="delete-button-group flex gap-2">
            <DialogClose asChild>
              <Button
                ...
                onClick={() => DeleteRecruitment(recruitmentID)}
              >
                Delete
              </Button>
            </DialogClose>
