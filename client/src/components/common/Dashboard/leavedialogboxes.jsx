...
import {
  ...
  HandlePostHRLeaves,
  HandlePatchHRLeaves,
  HandleDeleteHRLeaves,
} from "../../../redux/Thunks/HRLeavePageThunk.js";
...
export const CreateLeaveDialogBox = () => {
  ...
  const CreateLeave = async () => {
    const result = await dispatch(
      HandlePostHRLeaves({ apiroute: "CREATE", data: formdata }),
    );
    if (result.meta.requestStatus === "fulfilled") {
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: result.payload?.message || "Failed to create leave.",
      });
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        className="bg-blue-600 text-white font-medium px-4 py-2 rounded-xl hover:bg-blue-700 transition shadow-md"
        onClick={() => {
      ...
      <DialogContent className="max-w-[95vw] lg:max-w-[50vw] 2xl:max-w-[40vw]">
        <div className="create-leave-container flex flex-col gap-5">
          ...
          <div className="form-container grid md:grid-cols-2 min-[250px]:grid-cols-1 gap-4">
            <div className="form-group flex flex-col gap-3">
              ...
              <div className="label-input-field flex flex-col gap-1">
                ...
                <textarea
                  id="reason"
                  className="border-2 border-gray-700 rounded px-2 py-1 h-[100px] resize-none"
                  name="reason"
          ...
          <div className="add-button flex items-center justify-center gap-3">
            {!isFormValid ? (
              <Button
                className="bg-blue-600 border-2 border-blue-600 px-4 py-2 text-white font-bold rounded-lg hover:bg-white hover:text-blue-600"
                onClick={() =>
                  toast({
                    variant: "destructive",
                    title: "Error!",
            ...
            <DialogClose asChild>
              <Button
                className="bg-red-700 border-2 border-red-700 px-4 py-2 text-white font-bold rounded-lg hover:bg-white hover:text-red-700"
                onClick={() => clearForm()}
              >
                Cancel
              </Button>
...
export const ViewLeaveDialogBox = ({ leaveData, children }) => {
          ...
          <div className="leave-all-details grid lg:grid-cols-2 min-[250px]:gap-2 lg:gap-10">
            <div className="details-group-1 flex flex-col gap-3">
              ...
              <div className="label-value-pair flex items-center gap-2">
                ...
                <p className="md:text-sm xl:text-lg">
                  {calcDays(leaveData.startdate, leaveData.enddate)} Days
                </p>
              </div>
            </div>
            <div className="details-group-2 flex flex-col gap-3">
              <div className="label-value-pair flex items-center gap-2">
                ...
                <p
                  className={`md:text-sm xl:text-lg font-bold ${leaveData.status === "Approved" ? "text-green-700" : leaveData.status === "Rejected" ? "text-red-700" : "text-amber-700"}`}
                >
                  {leaveData.status}
                </p>
              </div>
              <div className="label-value-pair flex items-center gap-2">
                ...
                <p className="md:text-sm xl:text-lg">
                  {leaveData.approvedby
                    ? `${leaveData.approvedby.firstname} ${leaveData.approvedby.lastname}`
                    : "Not yet approved"}
                </p>
              </div>
...
export const ApproveRejectLeaveDialogBox = ({ leaveData, children }) => {
  const dispatch = useDispatch();
  const { toast } = useToast();
  const [open, setOpen] = useState(false);
  ...
  const HandleAction = async () => {
    ...
    if (result.meta.requestStatus === "fulfilled") {
      toast({
        title: "Success!",
        description: `Leave ${action.toLowerCase()} successfully.`,
      });
      dispatch(HandleGetHRLeaves({ apiroute: "GETALL" }));
      setOpen(false);
    } else {
      toast({
        variant: "destructive",
        title: "Error!",
        description: result.payload?.message || "Action failed.",
      });
    }
  };
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="leave-summary border-2 border-gray-300 rounded-lg p-4 flex flex-col gap-2">
            <p>
              ...
              {leaveData.employee?.firstname || ""}{" "}
              {leaveData.employee?.lastname || ""}
            </p>
            <p>
              <span className="font-bold">Title:</span> {leaveData.title}
            </p>
            <p>
            ...
            <p>
              <span className="font-bold">Reason:</span> {leaveData.reason}
            </p>
          </div>
          <div className="action-buttons flex justify-center gap-4">
            <Button
              className={`px-6 py-2 text-white font-bold rounded-lg border-2 ${action === "Approved" ? "bg-green-700 border-green-700" : "bg-white border-green-700 text-green-700"}`}
              onClick={() => setAction("Approved")}
            >
              {action === "Approved" ? "✓ Approved" : "Approve"}
            </Button>
            <Button
              className={`px-6 py-2 text-white font-bold rounded-lg border-2 ${action === "Rejected" ? "bg-red-700 border-red-700" : "bg-white border-red-700 text-red-700"}`}
          ...
          <div className="submit-button flex justify-center">
            {action ? (
              <Button
                className="bg-blue-600 border-2 border-blue-600 px-6 py-2 text-white font-bold rounded-lg hover:bg-white hover:text-blue-600"
              ...
              <Button
                className="bg-gray-400 border-2 border-gray-400 px-6 py-2 text-white font-bold rounded-lg cursor-not-allowed"
                disabled
              >
...
export const DeleteLeaveDialogBox = ({ leaveID, children }) => {
  const dispatch = useDispatch();
  const { toast } = useToast();

  const DeleteLeave = async (LID) => {
    const result = await dispatch(
      HandleDeleteHRLeaves({ apiroute: `DELETE.${LID}` }),
    );
    if (result.meta.requestStatus === "fulfilled") {
      toast({
        title: "Success!",
      ...
      toast({
        variant: "destructive",
        title: "Error!",
        description: result.payload?.message || "Failed to delete.",
      });
    }
  };
    ...
    <Dialog>
      ...
      <DialogContent className="max-w-[95vw] lg:max-w-[35vw] 2xl:max-w-[30vw]">
        <div className="flex flex-col justify-center items-center gap-4">
          <p className="text-lg font-bold min-[250px]:text-center">
            Are you sure you want to delete this leave record?
          </p>
          <div className="delete-button-group flex gap-2">
            <DialogClose asChild>
              <Button
                className="btn-sm border-2 min-[250px]:px-2 min-[250px]:py-1 sm:px-1 sm:py-0.5 xl:px-2 xl:py-1 rounded-md bg-red-700 border-red-700 hover:bg-transparent hover:text-red-700 text-white font-bold"
            ...
            <DialogClose asChild>
              <Button className="btn-sm border-2 min-[250px]:px-2 min-[250px]:py-1 sm:px-1 sm:py-0.5 xl:px-2 xl:py-1 rounded-md bg-green-700 border-green-700 hover:bg-transparent hover:text-green-700 text-white font-bold">
                Cancel
              </Button>
            </DialogClose>
          </div>
